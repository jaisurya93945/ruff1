...
tools/           build-manifest · serve · gen-portraits · gen-icons · aura-worker
...
**To use your own art** — the bundled portraits are generated vectors, plain
by nature because they are code rather than drawings. Three ways to replace
them, checked in this order:

1. **In the app.** Theme picker → hover a card → **+**. Or just tap the
   character on the home screen. Stored in that browser's IndexedDB; nothing
   is uploaded. Works from your phone.
2. **`images/themes/overrides.json`** — permanent, every device:
   ```json
   { "sakura": { "src": "images/art/sakura.png", "focus": "50% 20%", "fit": "cover" } }
   ```
   `focus` is the point kept in frame when cropped, `fit` is `cover` or
   `contain`. A bare `"path"` string works too.
3. **Overwrite the SVG** at `images/themes/<id>.svg`.

Tall images (2:3 or 3:4) with the head in the upper third frame best.
